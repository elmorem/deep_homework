...
from homework_researcher.memory.embeddings import Memory
import asyncio
import os
...
async def main():
    ...
    try:
        ...
        print(
            f"cfg.embedding_provider: {getattr(cfg, 'embedding_provider', 'Not set')}"
        )
        ...
        memory = Memory(
            embedding_provider=cfg.embedding_provider, model=cfg.embedding_model
        )
        ...
        test_text = (
            "This is a test sentence to verify embeddings are working correctly."
        )
